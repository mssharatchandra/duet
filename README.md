...
**Status: Phase 2 — hybrid SDR agent working end-to-end, CI + evals live.** Moshi holds the conversation in real time (step p95 **51 ms** vs the 80 ms/frame budget on an Apple M5); an async Gemini Flash brain supplies facts and objection handling, injected into Moshi's speech via the text-stream hook without ever blocking the audio loop (~1.1 s brain latency, fully masked). Reasoning quality is gated in CI by a golden eval (**92.7%**, gate 90%). Run it: [`agent/README.md`](agent/README.md) · numbers + method: [docs/DECISIONS.md](docs/DECISIONS.md). Nothing else below this line is a claim yet. Every claim that appears here later (latency, naturalness delta, cost per minute) will link to a reproducible benchmark in [`/eval`](eval/), or it won't appear at all.
