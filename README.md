...
**Status: Phase 3 — measured.** The full-duplex core (Moshi) and a cascaded baseline (faster-whisper → Gemini → Piper) ran the same 10 SDR scenarios with the same simulated caller. Result, published honestly in both directions ([method + numbers](eval/bench/RESULTS.md)): Duet's median response handoff is **240 ms vs the cascade's 1,880 ms (~8×)** and it backchannels while you talk — but its takeover rate is worse (0.24 vs 0.00) and its p95 tail slower. **Human naturalness delta: not yet measured** ([blind-eval protocol ready](docs/BLIND_EVAL.md)). Reasoning quality is CI-gated (golden eval ≥90%, currently 97.6%); the observability stack (Langfuse + Grafana + Prometheus) is in [`infra/`](infra/), CI-verified. Nothing else below this line is a claim.
