...
**Status: Phase 1 — local full-duplex proof of concept, working.** Run it: [`agent/README.md`](agent/README.md). Measured on an Apple M5: steady-state model step p95 **51 ms** against the 80 ms/frame real-time budget, ~5 GB peak memory ([numbers + method](docs/DECISIONS.md)). Nothing else below this line is a claim yet. Every claim that appears here later (latency, naturalness delta, cost per minute) will link to a reproducible benchmark in [`/eval`](eval/), or it won't appear at all.
